...
const sortStarWarsCharacters = (starWarsArr) => {
  // return starWarsArr.sort((starWarsArr.height => str (a,b) a-b))
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    let newItem = item.slice(item.indexOf(' ')+1);
    let deltedItem = newItem.slice(newItem.indexOf(' ')+1);
    result.push(deltedItem);
  });
  // Solution code here...
  return result;
};
